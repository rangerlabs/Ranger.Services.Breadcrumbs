...
namespace Ranger.Services.Breadcrumbs.Data
{
    public class BreadcrumbsRepository : IBreadcrumbsRepository
    {
        ...
        public async Task<IList<ConcurrentBreadcrumbResult>> UpsertGeofenceStates(string tenantId, Guid projectId, string deviceId, IEnumerable<Guid> geofenceIds, DateTime recordedAt)
        {
            ...
            using var cmd = new NpgsqlCommand("SELECT * FROM upsert_device_geofence_states(@tenant_id, @project_id, @device_id, @geofence_ids, @recorded_at)", connection);
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    concurrentBreadcrumbResults.Add(new ConcurrentBreadcrumbResult(reader.GetGuid(0), reader.GetString(1), reader.GetGuid(2), Enum.Parse<GeofenceEventEnum>(reader.GetInt32(3).ToString())));
                }
                reader.Close();
